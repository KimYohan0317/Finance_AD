...
def preprocessing_df(path, pran_name):
    ...
    df_fds = df_fds[df_fds['new_업태'] == pran_name]
    
    # 50% 랜덤 샘플링 (seed = 42)
    df_sampled = df_norm.sample(n=int(len(df_norm)*0.5), random_state=42)
    df_sampled = df_sampled[df_sampled['new_업태'] == pran_name]
    ...
    df_train = df_train[['원화환산금액', '시스템 등록일시', '가맹점아이디', '고객 카드번호 암호화값','label']]
    df_val = pd.concat([df_val_0, df_val_1])
    df_val = df_val[['원화환산금액', '시스템 등록일시', '가맹점아이디', '고객 카드번호 암호화값','label']]
    df_test = pd.concat([df_test_0, df_test_1])
    df_test = df_test[['원화환산금액', '시스템 등록일시', '가맹점아이디', '고객 카드번호 암호화값','label']]
...
def grid_search_and_evaluate(models, param_grids, X_train, y_train, X_test, y_test, pran_name):
    for model_name, model in models.items():
        ...
        if model_name == 'Random Forest': 
            ...
            # 텍스트 파일로 저장
            with open(f'/dshome/ddualab/yohan/finance/result/업태별/{model_name}_rule_tree_{p_name}_{today}.txt', 'w') as f:
                f.write(tree_rules)
        ...
        for thrsh in threshold:
            ...
            plt.ylabel('True')
            plt.savefig(f'/dshome/ddualab/yohan/finance/result/업태별/{model_name}_confusion_matrix_thrsh_{thrsh}_{p_name}_{today}.png')
            plt.close()
...
available_pran_name = ['도소매','서비스','General_e_comm','통신업']
# 데이터 전처리 및 모델 학습

for p_name in available_pran_name:
    X_train, X_val, X_test, y_train, y_val, y_test = preprocessing_df('/dshome/ddualab/yohan/finance/data_csv/total_df_align_KRW_1021.csv', pran_name=p_name)
    grid_search_and_evaluate(models, param_grids, X_train, y_train, X_test, y_test, pran_name=p_name)
