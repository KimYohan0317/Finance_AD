...
def preprocessing_df(path):
    df = pd.read_csv(path)
    df = df[['원화환산금액', '시스템 등록일시', '가맹점아이디', '고객 카드번호 암호화값','label']]
    df['시스템 등록일시'] = pd.to_datetime(df['시스템 등록일시'])
    df['시스템 등록일시'] = df['시스템 등록일시'].astype(np.int64) // 10**9
    df['가맹점아이디'] = le.fit_transform(df['가맹점아이디'])
...
# 그리드서치 및 평가 함수
def grid_search_and_evaluate(models, param_grids, X_train, y_train, X_test, y_test):
    for model_name, model in models.items():
        print(f"Grid search for {model_name}")
        logging.info(f"Grid search for {model_name}")
        ...
        if model_name == 'Random Forest': 
            ...
            # 텍스트 파일로 저장
            with open(f'/dshome/ddualab/yohan/finance/result/all_private_trade/{model_name}_rule_tree_{today}.txt', 'w') as f:
                f.write(tree_rules)
        ...
        for thrsh in threshold:
            ...
            plt.ylabel('True')
            plt.savefig(f'/dshome/ddualab/yohan/finance/result/all_private_trade/{model_name}_confusion_matrix_thrsh_{thrsh}_{today}.png')
            plt.close()

            # 성능 지표 출력
            print(f"{model_name}_threshold_{thrsh} - Accuracy: {accuracy_score(y_test, y_pred)}, F1-Score: {f1_score(y_test, y_pred)}")
            logging.info(f"{model_name}_threshold_{thrsh} - Accuracy: {accuracy_score(y_test, y_pred)}, F1-Score: {f1_score(y_test, y_pred)}")
...
# 데이터 전처리 및 모델 학습
X_train, X_val, X_test, y_train, y_val, y_test = preprocessing_df('/dshome/ddualab/yohan/finance/data_csv/total_df_align_KRW_1021.csv')
grid_search_and_evaluate(models, param_grids, X_train, y_train, X_test, y_test)
